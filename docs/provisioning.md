...
VM. Account and service provisioning scripts are present and fixture-tested, but CI still does not
run them against a real Windows VM.
...
The contracts and scripts intentionally stop before real provisioning execution. Later Phase 3 work
will add fake executors, real QGA and guest-service invocation, snapshot operations, health
