...
export const BOOTSTRAP_TOOLS: readonly CrucibleToolDefinition[] = [
  ...
  { name: "vm_mouse_move", description: "Move the VM mouse pointer to absolute coordinates." },
  { name: "vm_mouse_click", description: "Click a VM mouse button at optional coordinates." },
  { name: "vm_mouse_double_click", description: "Double-click a VM mouse button." },
  { name: "vm_mouse_drag", description: "Drag the VM mouse pointer between coordinates." },
  { name: "vm_key_press", description: "Send a key or key chord to the VM display." },
  { name: "vm_type_text", description: "Type literal text into the VM display." },
  {
    name: "network_status",
...
function registerVmTools(
  ...
  server.registerTool(
    ...
    {
      title: "Type VM text",
      description: "Type literal text into the VM display with an optional per-character delay.",
      ...
      try {
        ...
        if (stdout === "") {
          const stderr = Buffer.from(result.stderrBase64 ?? "", "base64")
            .toString("utf8")
            .trim();
          throw new Error(stderr || "ProcMon stop produced no JSON output");
        }
        return toJsonContent({ ok: true, result: JSON.parse(stdout) as unknown, auditLogPath });
      } catch (error) {
        return toJsonContent({
