...
public class CrucibleMemory {
  ...
  static byte[] PatternBytes(string p){ bool hex=p.Length%2==0; foreach(char c in p){ if(!Uri.IsHexDigit(c)){ hex=false; break; } } if(hex){ var b=new byte[p.Length/2]; for(int i=0;i<b.Length;i++) b[i]=Convert.ToByte(p.Substring(i*2,2),16); return b; } return Encoding.ASCII.GetBytes(p); }
